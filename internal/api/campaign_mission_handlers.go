...
func (s *Server) handleArmCampaignMission(c fiber.Ctx) error {
	...
	output := fmt.Sprintf("Running — a multi-trade ANNY Basic mission on %s (testnet). ANNY will auto-open and close capped entries toward %s USDT, stopping on the target, two consecutive losses, %d trades, or when the plan window ends at %s. Each entry uses your active testnet key with protective exits and a timed close. This one confirmation authorizes the whole series. Not financial advice.",
